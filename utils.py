...
def defect_objects(model, frame, class_names, display_width):
    ...
    for r in results:
        if r.marks:
            marks = r.marks.data.cpu().numpy()
            for i, mask in enumerate(marks):
                mask_resized = cv2.resize(mask, (resized_frame.shape[1], resized_frame.shape[0]), interpolation=cv2.interpolate.INTER_NEAREST)
        ...
        if r.boxes:
            for box in r.boxes:
                ...
                cv2.putText(resized_frame, label, (x1, max(y1 - 10, 0)), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0, 255, 0), 3)
